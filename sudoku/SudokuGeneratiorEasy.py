...
topGeneratorEasy = Tk()
topGeneratorEasy.title("Sudoku Easy")
topGeneratorEasy.config(width=550, height=380)
topGeneratorEasy.maxsize(width=550, height=380)

bgImage = Image.open('images\\purple-flowers.png')
displayBgImage = ImageTk.PhotoImage(bgImage)
lbBg = Label(topGeneratorEasy, image=displayBgImage).place(x=0, y=0)

def update_timer():
    current_time = int(timer_label["text"])
    current_time += 1
    timer_label["text"] = str(current_time)
    topGeneratorEasy.after(1000, update_timer)
...
def check_correctness(row, col):
    ...
    elif sudoku_grid_data[row][col] != original_sudoku[row][col]:
        draw.create_rectangle(
            col * 32 + 10,
            row * 32 + 10,
            (col + 1) * 32 + 10,
...
draw = Canvas(topGeneratorEasy, width=305, height=305, bg="#FFE4D1")
...
entry_row = Entry(topGeneratorEasy)
entry_row.place(x=110, y=350)
entry_row_label = Label(topGeneratorEasy, text="Row:")
entry_row_label.place(x=80, y=350)

entry_col = Entry(topGeneratorEasy)
entry_col.place(x=210, y=350)
entry_col_label = Label(topGeneratorEasy, text="Column:")
entry_col_label.place(x=160, y=350)

entry_num = Entry(topGeneratorEasy)
entry_num.place(x=310, y=350)
entry_num_label = Label(topGeneratorEasy, text="Number:")
entry_num_label.place(x=260, y=350)

fill_button = Button(topGeneratorEasy, text="Fill", command=fill_cell)
fill_button.place(x=410, y=346)

timer_label = Label(topGeneratorEasy, text="0", font=("Helvetica", 15))
timer_label.place(x=30, y=10)

update_timer()

menu_bar = Menu(topGeneratorEasy, font=("Arial", 10))
topGeneratorEasy.config(menu=menu_bar)
