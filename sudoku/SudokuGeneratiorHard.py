...
topGeneratorHard = Tk()
topGeneratorHard.title("Sudoku Easy")
topGeneratorHard.config(width=550, height=380)
topGeneratorHard.maxsize(width=550, height=380)

bgImage = Image.open('images\\purple-flowers.png')
displayBgImage = ImageTk.PhotoImage(bgImage)
lbBg = Label(topGeneratorHard, image=displayBgImage).place(x=0, y=0)

def update_timer():
    current_time = int(timer_label["text"])
    current_time += 1
    timer_label["text"] = str(current_time)
    topGeneratorHard.after(1000, update_timer)
...
draw = Canvas(topGeneratorHard, width=305, height=305, bg="#FFE4D1")
...
entry_row = Entry(topGeneratorHard)
entry_row.place(x=110, y=350)
entry_row_label = Label(topGeneratorHard, text="Row:")
entry_row_label.place(x=80, y=350)

entry_col = Entry(topGeneratorHard)
entry_col.place(x=210, y=350)
entry_col_label = Label(topGeneratorHard, text="Column:")
entry_col_label.place(x=160, y=350)

entry_num = Entry(topGeneratorHard)
entry_num.place(x=310, y=350)
entry_num_label = Label(topGeneratorHard, text="Number:")
entry_num_label.place(x=260, y=350)

fill_button = Button(topGeneratorHard, text="Fill", command=fill_cell)
fill_button.place(x=410, y=346)

timer_label = Label(topGeneratorHard, text="0", font=("Helvetica", 15))
timer_label.place(x=30, y=10)

update_timer()

menu_bar = Menu(topGeneratorHard, font=("Arial", 10))
topGeneratorHard.config(menu=menu_bar)
